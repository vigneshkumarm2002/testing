...
const Table = ({ data, pagination = true }) => {
  ...
  const onViewClick = (fileDetails) => {
    ...
    //const budgetHeaderNames = fileDetails.budgetHeaderNames.map(item => item.budgetHeader); // Extract budget headers


    //     const budgetHeaderNames = fileDetails.budgetHeaderNames.map((item, index) => (                       //Vignesh 
    //       <h1 className={`border px-2 py-1 border-gray-200 rounded-md ${index !== 0 && "mt-2" }`}>
    //         { item.budgetHeader } {" "}
    //         Challan : { item.challanFee } {" "}
